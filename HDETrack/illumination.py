...
class Illumination(BaseVideoDataset):
    def __init__(self, root=None, image_loader=jpeg4py_loader, split=None, seq_ids=None, data_fraction=None):
        ...
        root = env_settings().illumination_dir if root is None else root
    ...
    def _get_frame_path(self, seq_path, frame_id):
        if os.path.exists(os.path.join(seq_path, '{:04d}.png'.format(frame_id))):
            return os.path.join(seq_path, '{:04d}.png'.format(frame_id))
        else:
            return os.path.join(seq_path, '{:04d}.bmp'.format(frame_id))
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        frame_event_list = [np.zeros((4096, 19), dtype=np.float32) for _ in frame_ids]
