...
class Tracker:
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        # Flash detection for the init frame (= frame 1 in 1-indexed output).
        if isinstance(seq.frames[0], str):
            raw0 = cv.imread(seq.frames[0])
            ch0, ch1 = raw0[..., 0], raw0[..., 1]
            activity_ratio0 = np.count_nonzero(cv.bitwise_or(ch0, ch1)) / (raw0.shape[0] * raw0.shape[1])
            if activity_ratio0 > 0.2:
                output['flash_frames'].append(1)
        ...
        init_bb = init_info.get('init_bbox')
        init_kalman_center = (
            init_bb[0] + 0.5 * init_bb[2],
            init_bb[1] + 0.5 * init_bb[3],
        ) if init_bb is not None else None
        init_default = {'target_bbox': init_bb,
                        'time': time.time() - start_time,
                        'kalman_center': init_kalman_center}
        if tracker.params.save_all_boxes:
        ...
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            ...
            # Flash detection: read the raw BGR file (same as the offline notebook).
            # _read_image returns RGB, which would swap the B and R channels and
            # change which polarity ch0 corresponds to.
            # frame_num+1 → 1-indexed, where the init frame (seq.frames[0]) = 1.
            is_flash = False
            if isinstance(frame_path, str):
                raw = cv.imread(frame_path)
                ch0, ch1 = raw[..., 0], raw[..., 1]
                activity_ratio = np.count_nonzero(cv.bitwise_or(ch0, ch1)) / (raw.shape[0] * raw.shape[1])
                is_flash = activity_ratio > 0.2
                if is_flash:
                    output['flash_frames'].append(frame_num + 1)

            start_time = time.time()

            info = seq.frame_info(frame_num)
            info['previous_output'] = prev_output
            info['flash'] = bool(is_flash)
