...
class SDTrack(BaseTracker):
    def __init__(self, params, dataset_name):
        ...
        self.z_dict1 = {}
        # Counts how many more post-flash frames should still use redetection.
    ...
    def track(self, image, info: dict = None):
        if self.cfg.MODEL.T == 1:
            ...
            # Redetection gate: enable sliding-window search during flash frames
            # and for 3 frames after the flash ends. Otherwise fall back to the
            # fast local-only path.
            ...
            if use_redetect:
                # Window size = search_factor * max(bbox_w, bbox_h) of current state,
                # clamped to frame size. Used for both the local window and sliding
                # windows so every region shares the same resize_factor.
                win = int(self.params.search_factor * max(self.state[2], self.state[3]))
                win = max(1, min(win, H, W))
                step = max(1, int(win * 0.7))  # 50% overlap

                # Local window — centered on current bbox, clamped inside the frame.
                ...
                # Sliding grid of windows across the full frame.
                ...
                # Build the batch: crop → resize → preprocess for every window.
                ...
                # Fast local-only path: one crop around the previous bbox.
